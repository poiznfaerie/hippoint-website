...
function productCardHTML(p) {
  ...
  const productImage = product.imageUrl
    ? `<img src="${product.imageUrl}" alt="${product.name}" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" /><div class="product-img-1" style="display:none">${product.emoji}</div><div class="product-img-2" style="display:none">${product.emoji2 || product.emoji}</div>`
    : `<div class="product-img-1" aria-hidden="true">${product.emoji}</div><div class="product-img-2" aria-hidden="true">${product.emoji2 || product.emoji}</div>`;
...
function renderHomeProducts() {
  ...
  if (USE_JSON && typeof dbProducts !== 'undefined' && dbProducts.length > 0) {
    console.log("Products:", dbProducts.length, "first img:", dbProducts[0]?.image_url); console.log("Rendering products:", dbProducts.slice(0,2).map(p => normalizeProduct(p))); el.innerHTML = dbProducts.slice().sort((a,b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 8).map(p => productCardHTML(normalizeProduct(p))).join('');
  } else {
